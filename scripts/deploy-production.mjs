...
if (!project) {
  console.error('[deploy:error] CF_PAGES_PROJECT is required to run this script.');
  process.exit(1);
}

try {
  log('env', 'Validating required environment variables');
  execSync('node scripts/validate-env.mjs', { cwd: path.resolve(__dirname, '..'), stdio: 'inherit' });

  log('build', 'Building workspace');
  execSync('npm run build', { cwd: path.resolve(__dirname, '..'), stdio: 'inherit' });

  log('deploy', `Deploying ${dist} to project ${project} (branch ${branch})`);
  execSync(`npx wrangler pages deploy ${dist} --project-name ${project} --branch=${branch}`, {

...
function main() {
  ...
  const keysToPreserve = Object.keys(existing).filter(k => !Object.hasOwn(newSettings, k));
      for (const k of keysToPreserve) merged[k] = existing[k];
    } catch (e) {
      console.warn('Warning: failed to parse existing mcp-settings.json, overwriting.', e && e.message ? e.message : '');
    }
  }
  ...
  addFormats(ajv);
    ...
    console.warn('Schema validation skipped (error loading schema):', e.message);
  }

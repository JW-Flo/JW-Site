...
const quickQueries = [
  {
    ...
    context: "Current IT automation market trends and opportunities",
  },
  {
    step: "quick_competitor_query",
    context: "Top IT automation competitors in 2024",
  },
  {
    step: "quick_sentiment_query",
    context: "IT professional sentiment on automation tools",
  },
...
async function runQuickResearch() {
  ...
  console.log(
    `Successful queries: ${results.filter((r) => r.success).length}/${
      quickQueries.length
    }`
  );
  console.log(
    `Average execution time: ${Math.round(
      results
        .filter((r) => r.success)
        ...
        result.data.insights.slice(0, 2).forEach((insight) => {
          console.log(
            `  • ${insight.summary || insight} (${
              insight.sentiment?.label || "N/A"
            })`
          );
        });
      } else if (result.data.topCompetitors) {
        console.log(
          `  • Top: ${result.data.topCompetitors.slice(0, 3).join(", ")}`
        );
      } else if (result.data.overallSentiment) {
        const sentimentStr = Object.entries(result.data.overallSentiment)
          .map(([s, c]) => `${s}: ${c}`)
          .join(", ");

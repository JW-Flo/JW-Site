...
function logDryRun() {
  console.log(
    "[DRY RUN] Would generate a new key and execute: npx wrangler pages secret put CONSENT_ADMIN_KEY --project-name jw-site"
  );
  if (!noStore) {
    console.log(
      `[DRY RUN] Would create directory ${secretsDir} (if missing) and write:`
    );
    console.log(`  - ${latestFile} (0600) containing the new key`);
    console.log(
      `  - ${historyFile} (append JSON line with timestamp + partial key)`
    );
  } else {
    console.log("[DRY RUN] --no-store set: would NOT write local secret files");
  }
  if (deploy)
    console.log("[DRY RUN] Would also run: npm run build && npm run deploy");
}
...
function storeKey(key) {
  ...
  try {
    ...
    console.log(
      `Stored key locally at ${latestFile} (permissions 600). History appended.`
    );
  } catch (e) {
...
function setSecretAndMaybeDeploy(key) {
  ...
  if (deploy) {
    ...
    console.log(
      "Run `npm run build && npm run deploy` to activate the new key."
    );
  }
}

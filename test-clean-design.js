  ...
  const tests = [
    {
      ...
      test: async () => {
        ...
        const success = response.body.includes('text-blue-400') &&
        ...
        return { success, details: 'Professional colors detected, neon colors removed' };
      }
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('Joe Whittle') &&
        ...
        return { success, details: 'Professional hero content without cyberpunk elements' };
      }
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('Gaming Mode') &&
                       response.body.includes('Available') &&
                       !response.body.includes('READY PLAYER ONE') &&
                       !response.body.includes('JACK IN');
        return { success, details: 'Subtle game trigger without cyberpunk styling' };
      }
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('Inter') &&
    ...
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('Explore Interactive Demo') &&
        ...
        return { success, details: 'Professional button text without cyberpunk styling' };
      }
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('Security Incidents Resolved') &&
        ...
        return { success, details: 'Professional stats without all-caps styling' };
      }
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = !response.body.includes('animate-ping') &&
    ...
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('slate-900') &&
                       response.body.includes('slate-800') &&
                       !response.body.includes('radial-gradient') &&
                       !response.body.includes('neon');
        return { success, details: 'Clean slate background without cyberpunk effects' };
      }
    }
  ];

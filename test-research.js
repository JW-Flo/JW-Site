...
async function testResearchEngine() {
  ...
  try {
    ...
    const quickResult = await handlers.quick_market_query(
      "IT automation market trends"
    );
    ...
    const marketResult = await handlers.market_research(
      "Enterprise IT automation platform"
    );
    ...
    const competitiveResult = await handlers.competitive_intelligence(
      "IT automation competitors"
    );
    console.log(
      "✅ Competitive intelligence:",
      competitiveResult?.topic || "Success"
    );

    console.log("\nTesting atlasit_research...");
    const atlasitResult = await handlers.atlasit_research(
      "IT automation platform market value"
    );

...
const SITE_URL = 'https://855e99b5.jw-site-7q2.pages.dev';
...
async function testStyling() {
  ...
  const tests = [
    ...
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('READY PLAYER ONE') &&
    ...
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('<section class="text-center py-12 md:py-20 relative overflow-hidden') &&
    ...
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('text-cyan-400') &&
    ...
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('mobile-menu') &&
    ...
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('fonts.googleapis.com') &&
    ...
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('INCIDENTS RESOLVED') &&
    ...
    },
    {
      ...
      test: async () => {
        const response = await makeRequest(SITE_URL);
        const success = response.body.includes('game-trigger') &&

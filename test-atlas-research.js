  ...
  try {
    ...
    const response = await fetch("http://localhost:5050/mcp", {
      ...
      body: JSON.stringify({
        ...
        context:
          "AtlasIT market value, valuation, competitive position, and growth potential",
      }),
    });
    ...
    if (data.insights) {
      ...
      if (data.insights.rss && data.insights.rss.length > 0) {
        data.insights.rss.slice(0, 3).forEach((insight, i) => {
          ...
          if (insight.trends && insight.trends.topTrends) {
            console.log(
              `   Top trends: ${insight.trends.topTrends
                .map((t) => t.trend)
                .join(", ")}`
            );
          }
      ...
      [
        ...
        if (insight.trends && insight.trends.marketSize) {
          insight.trends.marketSize.forEach((size) => {
            console.log(
              `- ${size.raw} (Context: ${size.context.substring(0, 100)}...)`
            );
      ...
      if (marketMentions > 0) {
        console.log(
          `\nAverage market value mentioned: $${(
            totalMarketValue /
            marketMentions /
            1000000
          ).toFixed(0)}M`

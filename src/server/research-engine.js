...
class ResearchEngine {
  ...
  async performWebSearch(query, numResults = 10) {
    ...
    for (const result of allResults) {
      try {
        ...
        console.log(
          `Content extraction failed for ${result.url}:`,
          err.message
        );
        enrichedResults.push({
          ...result,
    ...
    return [
      {
        ...
        url: `https://www.crunchbase.com/search/funding-rounds/field/organizations/funding_total/${encodeURIComponent(
          query
        )}`,
  ...
  async generateResearchDocument(
    ...
    const allContent = searchResults
      ...
      .join(" ")
      .substring(0, 2000);
    document.executive_summary = await this.aiAnalyzer.summarizeText(
      allContent,
      200
    );
    ...
    if (analysisType === "market" || analysisType === "comprehensive") {
      const marketSection = await this.generateMarketAnalysis(
        topic,
        searchResults
      );
    ...
    if (analysisType === "competitive" || analysisType === "comprehensive") {
      const competitiveSection = await this.generateCompetitiveAnalysis(
        topic,
        searchResults
      );
    ...
    if (analysisType === "technology" || analysisType === "comprehensive") {
      const techSection = await this.generateTechnologyAnalysis(
        topic,
        searchResults
      );
    ...
    document.sections.push(
      await this.generateKeyFindings(topic, searchResults)
    );
  ...
  async generateMarketAnalysis(topic, searchResults) {
    const marketContent = searchResults
      .filter(
        (r) =>
    ...
    return {
      ...
      content: [
        ...
        `Market Sentiment: ${analysis.sentiment.label} (${Math.round(
  ...
  async generateCompetitiveAnalysis(topic, searchResults) {
    ...
    return {
      ...
      content: [
        ...
        `Differentiation Opportunities: ${this.identifyOpportunities(
          topic,
          competitorContent
        )}`,
      ],
  ...
  async generateTechnologyAnalysis(topic, searchResults) {
    const techContent = searchResults
      .filter(
        (r) =>
  ...
  async generateKeyFindings(topic, searchResults) {
    ...
    return {
      ...
      content: [
        ...
        `Primary Opportunities: ${this.identifyOpportunities(
          topic,
          allContent
        )}`,
        `Risk Factors: ${this.identifyRisks(allContent)}`,
        `Strategic Recommendations: ${this.generateRecommendations(
          topic,
          allContent
        )}`,
      ],
  ...
  assessCompetition(content) {
    if (content.toLowerCase().includes("highly competitive"))
      return "Highly competitive market";
    if (content.toLowerCase().includes("emerging"))
      return "Emerging market with growing competition";
  ...
  identifyOpportunities(topic, content) {
    ...
    if (content.toLowerCase().includes("gap"))
      opportunities.push("Market gaps identified");
    if (content.toLowerCase().includes("underserved"))
      opportunities.push("Underserved segments");
    if (content.toLowerCase().includes("innovation"))
  ...
  identifyChallenges(content) {
    ...
    if (content.toLowerCase().includes("security"))
      challenges.push("Security concerns");
    if (content.toLowerCase().includes("regulation"))
      challenges.push("Regulatory compliance");
    if (content.toLowerCase().includes("cost"))
      challenges.push("Cost optimization");
  ...
  identifyRisks(content) {
    ...
    if (content.toLowerCase().includes("economic"))
  ...
  assessDataQuality(searchResults) {
    const quality = searchResults.filter(
      (r) => r.extracted && r.content.length > 500
    ).length;

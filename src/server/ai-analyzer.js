  ...
  getFallbackResponse(model, error) {
    if (model.includes("bart-large-cnn") || model.includes("llama")) {
      return {
        result: [
          {
            summary_text: `AI summarization failed (${error}). Fallback: No summary generated.`,
          },
        ],
      ...
      return {
        result: {
          text: `AI output failed (${error}). Fallback: No response generated.`,
        },
        provider: "cloudflare-ai-gateway",
      };
    } else if (model.includes("whisper")) {
      return {
        result: {
          text: `AI transcription failed (${error}). Fallback: No transcription generated.`,
        },
        provider: "cloudflare-ai-gateway",
      };
    } else {
  ...
  parseResult(model, result) {
    if (model.includes("bart-large-cnn") || model.includes("llama")) {
      return {
        result: [
          {
            summary_text: result.response || result.result || result.text || "",
          },
        ],
        provider: "cloudflare-ai-gateway",
      };
  ...
  async extractEntities(text) {
    try {
      ...
      const companyPatterns = [
        /\b[A-Z][a-z]+\s+(?:Inc|Corp|LLC|Ltd|Co)\b/g,
        /\b[A-Z]{2,}\b/g,
      ];

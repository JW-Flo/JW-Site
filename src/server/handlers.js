...
const handlers = {
  ...
  market_research: async (context) => {
    ...
    try {
      const searchResults = await researchEngine.performWebSearch(
        `${topic} market size revenue growth`,
        12
      );
      const researchDocument = await researchEngine.generateResearchDocument(
        topic,
      ...
      return {
        ...
        market_insights: {
          ...
          market_data_found: searchResults.filter(
            (r) =>
  ...
  competitive_intelligence: async (context) => {
    ...
    try {
      const searchResults = await researchEngine.performWebSearch(
        `${topic} competitive analysis companies`,
        10
      );
      const researchDocument = await researchEngine.generateResearchDocument(
        topic,
      ...
      return {
        ...
        competitive_analysis: {
          ...
          market_positioning:
            "Analysis based on web research and AI extraction",
        },
        provider: "atlasit-research-engine",
      };
    } catch (err) {
  ...
  quick_market_query: async (context) => {
    ...
    try {
      ...
      const marketAnalysis = await researchEngine.generateMarketAnalysis(
        topic,
        searchResults
      );
  ...
  atlasit_research: async (context) => {
    ...
    const query = `${
      context || "IT automation platform"
    ...
    try {
      ...
      const atlasitInsights = {
        ...
        market_opportunity:
          "Growing demand for integrated IT management solutions",
        competitive_advantage:
          "Comprehensive platform approach with AI-enhanced analytics",
        key_differentiators: [
      ...
      return {
        ...
        market_validation: {
          sources_analyzed: searchResults.length,
          market_demand_indicators: searchResults.filter(
            (r) =>
  ...
  technology_trends: async (context) => {
    ...
    try {
      const searchResults = await researchEngine.performWebSearch(
        `${topic} 2024 predictions innovation`,
        10
      );
      ...
      return {
        ...
        trends_analysis: {
          total_sources: searchResults.length,
          tech_trends_identified: searchResults.filter(
            (r) =>
  ...
  ai_summarize: async (context, accumulatedData) => {
    ...
    const text =
      context || accumulatedData?.text || "No text provided for summarization";
  ...
  context_ingestion: async (context) => {
    ...
    try {
      ...
      const contextAnalysis = await researchEngine.generateMarketAnalysis(
        context,
        searchResults
      );

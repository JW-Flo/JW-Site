...
(function(global){
  ...
  function loadJSON(key, fallback){
    try { const v = global.localStorage.getItem(key); return v ? JSON.parse(v) : fallback; } catch { return fallback; }
  }
  function saveJSON(key, value){
    try { global.localStorage.setItem(key, JSON.stringify(value)); } catch { /* ignore write errors */ }
  }

  function getStats(){
    // Derive coarse stats from existing leaderboard + totals already stored by GameManager
    const totalScore = parseInt(global.localStorage.getItem('retroArcadeTotalScore')||'0',10) || 0;
    const achievementsRaw = global.localStorage.getItem('retroArcadeAchievements');
  const achievements = achievementsRaw ? Object.values(JSON.parse(achievementsRaw)).filter(Boolean).length : 0;
  ...
  const HINTS = {
    visitor: [
      { id: 'press-enter', text: 'Earn points to unlock classics. Press Enter to start.' },
      { id: 'score-unlocks', text: 'Score points to reveal new games. Asteroids is first.' }
    ],
    novice: [
      { id: 'unlock-pacman', text: 'Reach 150 total points to unlock the maze.' },
      { id: 'chain-bonus', text: 'Try different games in a row for a future bonus.' }
    ],
    competent: [
      { id: 'tetris-path', text: '300 total points opens the puzzle challenge.' },
      { id: 'final-tease', text: 'A final defense awaits at 500 points.' }
    ],
    advanced: [
      { id: 'secrets', text: 'Hidden progress grows by exploring unusual site areas.' },
      { id: 'robots', text: 'Even robots.txt can whisper clues.' }
    ]
  };

  function pickHint(list, seen){
    for (const h of list){ if (!seen[h.id]) return h; }
  ...
  function incrementSecretProgress(amount){
    try {
      const current = parseInt(global.localStorage.getItem(PROGRESS_KEY)||'0',10)||0;

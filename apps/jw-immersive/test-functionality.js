...
addTest("Homepage loads correctly", async () => {
  ...
  if (!html.includes("Cybersecurity Engineer"))
...
addTest("Guestbook page loads with form", async () => {
  ...
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const html = await response.text();
  if (!html.includes("guestbook-form"))
    throw new Error("Missing guestbook form");
});
...
addTest("Game score processor API responds", async () => {
  const response = await fetch(
    `${BASE_URL}/api/workflows/game-score-processor`
  );
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const data = await response.json();
...
addTest("GameManager.js loads", async () => {
  ...
  if (!content.includes("class GameManager"))
    throw new Error("Invalid GameManager file");
});
...
addTest("GameOverlay.js loads", async () => {
  ...
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const content = await response.text();
  if (!content.includes("class GameOverlay"))
    throw new Error("Invalid GameOverlay file");
});
...
addTest("Game files are accessible", async () => {
  ...
  for (const game of games) {
    ...
    if (!response.ok)
      throw new Error(`${game} not accessible: HTTP ${response.status}`);
  }
});
